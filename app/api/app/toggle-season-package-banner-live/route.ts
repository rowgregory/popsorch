...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    await createLog('info', 'Toggled season package banner live', {
    ...
    await createLog('error', `Toggling season package banner live failed: ${error.message}`, {
    ...
    return NextResponse.json(
      {
        success: false,
        error: 'Failed to toggle season package banner live'
      },
      { status: 500 }
    )
  } finally {
