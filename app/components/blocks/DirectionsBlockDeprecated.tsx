...
const DirectionsBlockDeprecated = () => {
  const [coordinates, setCoorinates] = useState<{ long: number; lat: number; address: string }>(venues.riverview)
  ...
  return (
    <div className="px-4 py-14 md:px-12 md:py-24">
      <div className="max-w-[516px] 760:max-w-[700px] 990:max-w-[960px] 1200:max-w-[1160px] 1400:max-w-1320 mx-auto w-full grid grid-cols-12 gap-y-16 md:gap-x-8 items-center relative">
        ...
        <div className="col-span-12 990:col-span-6">
          <div className="flex flex-col gap-y-10">
            <div className="text-[47px] 430:text-[60px] text-gunmetal dark:text-white font-bold mt-1">
              Get Directions to Our Venues
            </div>
            <span className="cursor-pointer" onClick={() => setCoorinates(venues.riverview)}>
              <AnimatedSectionHeader
                title="Riverview Performing Arts Center"
                isActive={coordinates.address === venues.riverview.address}
              />
            </span>
            <span className="cursor-pointer" onClick={() => setCoorinates(venues.scf)}>
              <AnimatedSectionHeader
                title="SCF Neel Performing Arts Center"
                isActive={coordinates.address === venues.scf.address}
              />
            </span>
...
export default DirectionsBlockDeprecated
