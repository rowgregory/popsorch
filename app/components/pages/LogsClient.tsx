        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-6">
          <div className="bg-surface-dark border border-border-dark max-w-4xl w-full max-h-[80vh] overflow-hidden flex flex-col">
            <div className="flex items-center justify-between p-4 border-b border-border-dark">
              ...
              <button onClick={() => setExpandedLog(null)} className="text-muted-dark hover:text-text-dark transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
                ...
                return (
                  <div>
                    <div className="mb-4">
                      ...
                      <p className="text-[10px] font-mono text-muted-dark">{new Date(log.createdAt).toLocaleString()}</p>
                    </div>
                    {log.metadata && (
  ...
  return (
    <div className="bg-surface-dark border border-border-dark">
      ...
      <div className="p-3 space-y-2 max-h-[calc(100vh-200px)] overflow-y-auto">
        {logs.map((log) => (
          <div key={log.id} className="p-3 bg-bg-dark border-l-2 border-border-dark hover:border-primary-dark transition-colors">
            <div className="flex items-start justify-between gap-2 mb-2">
              <span className={`text-[9px] font-mono tracking-[0.2em] uppercase px-1.5 py-0.5 ${colorClass}`}>{log.level}</span>
              <button onClick={() => onExpand(log.id)} className="text-muted-dark hover:text-text-dark transition-colors">
