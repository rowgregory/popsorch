...
export const StaffClient = ({ staff }) => {
  ...
  const handleOpenDrawer = (selectedMember: TeamMemberProps) => {
    ...
    dispatch(setOpenDrawer({ teamMember: selectedMember, teamList: combinedList, selectedIndex }))
  }
  ...
  return (
    <main id="main-content">
      ...
      <div className="relative">
        <div
          className="absolute inset-0 w-full h-full bg-no-repeat bg-center bg-cover opacity-35"
          ...
          <div className="max-w-[320px] 430:max-w-130 760:max-w-xl 990:max-w-200 1200:max-w-screen-1160 1590:max-w-screen-7xl mx-auto">
            ...
            <header className="w-full text-center flex flex-col items-center pt-32 pb-20 border-b border-white/10">
              ...
              <div className="flex items-center gap-3 430:gap-4 justify-center mb-4">
                ...
                <h1 className="text-4xl 430:text-5xl sm:text-6xl font-changa text-white leading-none">Our Staff</h1>
                <div className="w-8 430:w-16 h-px bg-blaze shrink-0" aria-hidden="true" />
              </div>
              ...
              <p className="font-lato text-white/50 text-sm 430:text-base max-w-xl leading-relaxed">
                Meet the dedicated team behind The Pops Orchestra of Bradenton and Sarasota.
              </p>
            </header>

            {/* Staff Grid */}
            <section aria-labelledby="staff-heading" className="py-20 pb-32">
              <h2 id="staff-heading" className="sr-only">
                Staff members
              </h2>
              <ul role="list" aria-label="Staff members" className="grid grid-cols-12 gap-y-6 760:gap-7 w-full">
                {sortedStaff?.map((teamMember: TeamMemberProps, index: number) => (
