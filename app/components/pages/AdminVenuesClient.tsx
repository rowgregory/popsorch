...
export const AdminVenuesClient = ({ data }) => {
  ...
  const handleDelete = async (venueId: string) => {
    try {
      ...
      router.refresh()
      dispatch(showToast({ message: 'Venue deleted successfully', type: 'success' }))
    } catch {
      dispatch(showToast({ message: 'Failed to delete venue', type: 'error' }))
    } finally {
      setDeletingId(null)
    }
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="min-w-0">
              <h1 className="text-2xl sm:text-3xl font-bold text-white">Venues</h1>
              <p className="text-neutral-400 text-sm sm:text-base mt-1">
                Manage performance venues and location details
              </p>
            </div>
          </div>
        </div>
