...
export default function TheCauldron({ data }) {
  const dispatch = useAppDispatch()
  const { pageSelectorModal, selectedCauldronFolder } = useDashboardSelector()
  const selectedFolderObj = cauldronFolders.find((folder) => folder.value === selectedCauldronFolder)
  ...
  const handleSave = async (content: any) => {
    ...
    try {
      if (data?.id) {
        ...
        await updatePage(selectedCauldronFolder, { content })
      } else {
        // Create new page
        await createPage({
          slug: selectedCauldronFolder,
          content
        })
      }
      ...
      dispatch(
        showToast({
          message: `${selectedCauldronFolder} page ${data?.id ? 'updated' : 'created'} successfully!`,
          type: 'success'
        })
      )
    } catch (error) {
