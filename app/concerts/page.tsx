...
import { containerVariants, itemVariants, searchBoxVariants } from '../lib/constants/motion'

const getConcertsByType = (concerts: ConcertProps[], type: string): ConcertProps[] => {
                    ...
                    <motion.div key="results" className="space-y-4 md:space-y-6" variants={containerVariants}>
                      {filteredConcerts.map((concert: ConcertProps, index: number) => (
                        <motion.div
                          ...
                          custom={index}
                        >
                          <PublicConcertRow concert={concert} index={index} />
                        </motion.div>
                      ))}
                    </motion.div>
                  )}
            ...
            >
              <div className="xl:sticky xl:top-24 space-y-6">
                      ...
                      concerts.length > 0 && (
                        ...
                        >
                          <p className="text-xs text-zinc-500 text-center">
                            {concerts.length} events available to search
                          </p>
                        </motion.div>
                      )
                    )}
                ...
                >
                  ...
                  {/* On Sale indicator */}
                  {filteredConcerts.filter((c) => c.isOnSale).length > 0 && (
                    <motion.div
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                    ...
                    >
                      ...
                      {filteredConcerts.filter((c) => c.isOnSale).length} on sale now
                    </motion.div>
                  )}
                </motion.div>
              </div>
